...
from quiz_app.forms import QuizForm, CreateUserForm, UserLoginForm, CreateStudentForm, CreateTeacherForm, ExaminationInfoForm
...
def exam_list_view(request):
    ...
    if exam_list:
        return render(request, 'exam_view.html', {'std_exam_info':exam_list , 'activation_message':activation_message})

    return render(request, 'exam_view.html', {'message':'There are no Qusetions available','activation_message':activation_message})
...
def quiz_view(request,id , pk=0):
    ...
    if UserQuizInfo.objects.filter(examination_info=std_exam_info,user_info=request.user.student).exists():
        ...
        # import pdb; pdb.set_trace()
        if (time < (timezone.now()-info.last().start_time)):
    ...
    def get_next_data(quiz_list_id):
        # quiz = Quiz.objects.fiDATABASES = {
#     'default': {
#         'ENGINE': 'django.db.backends.sqlite3',
#         'NAME': BASE_DIR / 'db.sqlite3',
#     }
# }lter(pk=pk, exam_info = std_exam_info).count()
        # if (quiz == 1):
    ...
    if request.method == 'POST' :
        ...
        quiz = get_next_data(pk)['quiz']
        options = get_next_data(pk)['options']
        pk = get_next_data(pk)['pk']
        next_pk = get_next_data(pk)['next_pk']
        ...
        if len(client_given)-1 >= 1:
            ...
            return render(request, 'index.html', {'quiz':quiz, 'options': options, 'next_pk':next_pk, 'error':'You have not provided any answer'})
    ...
    if pk == all_questions.count():
        ...
        return render(request, 'index.html', {'quiz':quiz, 'options': options, 'next_pk':next_pk })
...
def result_view(request):
    students_result = UserQuizInfo.objects.filter(exam_done=True).order_by('-num_of_correct_ans')[:30]
